...
func TestCheckResolvesMatchingAsset(t *testing.T) {
	client := &http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		return responseWithBody(http.StatusOK, `{
			...
			"assets":[
				{"name":"tideftp-linux-x86_64.tar.gz","browser_download_url":"https://example.com/download"},
				{"name":"tideftp-darwin-aarch64.tar.gz","browser_download_url":"https://example.com/other"},
	...
	if result.Latest.AssetName != "tideftp-linux-x86_64" {
...
func TestDownloadExtractsBinary(t *testing.T) {
	archive := testArchive(t, "tideftp-linux-x86_64", "#!/bin/sh\necho tide\n")
	sums := sha256Hex(archive) + "  tideftp-linux-x86_64.tar.gz\n"
	updater := &Updater{HTTPClient: archiveServer(archive, sums)}
	asset, err := updater.Download(ReleaseInfo{
		Version:      "v1.2.3",
		AssetName:    "tideftp-linux-x86_64",
...
func TestDownloadRejectsChecksumMismatch(t *testing.T) {
	archive := testArchive(t, "tideftp-linux-x86_64", "real")
	// Checksum of different content — must not install.
	sums := sha256Hex([]byte("tampered")) + "  tideftp-linux-x86_64.tar.gz\n"
	updater := &Updater{HTTPClient: archiveServer(archive, sums)}
...
func TestInstallFallsBackToUserLocalBinWhenCurrentDirNotWritable(t *testing.T) {
	...
	current := filepath.Join(currentDir, "tideftp")
	if err := os.WriteFile(current, []byte("old"), 0o755); err != nil {
		t.Fatalf("write current binary: %v", err)
	}
	newBinary := filepath.Join(dir, "downloaded")
	...
	target := filepath.Join(home, ".local", "bin", "tideftp")
	if result.RequiresManual {
...
func TestInstallTargetCheckDoesNotCreateUserLocalBin(t *testing.T) {
	...
	target, err := installTarget(filepath.Join(currentDir, "tideftp"), false)
	if err != nil {
		t.Fatalf("installTarget returned error: %v", err)
	}
	want := filepath.Join(home, ".local", "bin", "tideftp")
	if target != want {
...
// The asset names this builds must match, exactly, what the release workflow
// publishes — .github/workflows/release.yml builds the same four
// goos/arch pairs and tars them as "<binary>-<goos>-<arch>.tar.gz". A drift
// on either side means Check finds no asset for the running platform and the
// updater silently reports "no update" forever, which is the failure mode
// this pins down.
func TestAssetNameMatchesThePublishedReleaseAssets(t *testing.T) {
	for _, tc := range []struct{ goos, goarch, want string }{
		...
		updater := &Updater{GOOS: tc.goos, GOARCH: tc.goarch}
		got, err := updater.assetName()
		if err != nil {
			t.Fatalf("%s/%s: %v", tc.goos, tc.goarch, err)
		}
		if got != tc.want {
			t.Errorf("%s/%s asset = %q, want %q", tc.goos, tc.goarch, got, tc.want)
		}
	}

	// Anything else has no published build, and must say so rather than
	// guessing at a name that will 404.
	for _, tc := range []struct{ goos, goarch string }{
		{"windows", "amd64"},
		{"linux", "386"},
	} {
		if _, err := (&Updater{GOOS: tc.goos, GOARCH: tc.goarch}).assetName(); err == nil {
			t.Errorf("%s/%s: want an error, got none", tc.goos, tc.goarch)
		}
	}
}
